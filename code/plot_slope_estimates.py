...
def fit_mass_slope(
    ...
    if dof <= 0:
        return float(coef[1]), float("nan"), float(coef[0]), n

    mse = float(np.sum(resid**2) / dof)
    cov = mse * np.linalg.inv(design.T @ design)
    se = float(np.sqrt(cov[1, 1]))
    return float(coef[1]), se, float(coef[0]), n


def build_slope_summary(df: pd.DataFrame, min_rows: int) -> pd.DataFrame:
    global_b, global_se, global_intercept, global_n = fit_mass_slope(
        df["log_mass"].to_numpy(), df[LOG_TARGET].to_numpy()
    )

    rows: list[dict[str, object]] = []
    for class_name, group in df.groupby(CLADE_COL, sort=True):
        b, se, intercept, n = fit_mass_slope(
            group["log_mass"].to_numpy(), group[LOG_TARGET].to_numpy()
        )
...
def save_slope_plot(summary: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.set_xlabel("mass-scaling exponent b")
    ax.set_ylabel("Class")
    ax.set_title("Mass scaling exponent estimates")
...
def write_summary_table(summary: pd.DataFrame, output_path: Path) -> None:
    ...
    meta = pd.DataFrame(
        [
            {"scope": "global", "class": "(all)", "b": global_b, "se": global_se, "n_rows": global_n},
            {"scope": "reference", "class": "(all)", "b": REFERENCE_SLOPE, "se": np.nan, "n_rows": global_n},
        ]
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Plot class-level mass-scaling exponent b with 95% CI, "
            "plus reference lines at b = 0.75 and the global estimate."
        )
    )
    parser.add_argument(
        "--train",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/plots/slope_estimates.png"),
